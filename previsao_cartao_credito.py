...
""" arquivo = open("/home/brunohelghast/PROFISSIONAL/PYTHON/SCIKIT_LEARN/previsaoAprovacaoCartaoCredito/dados_limpos.csv","a")
for i in range(0,len(dados.iloc[:,3]),1):
    for j in range(0,18,1):
        if(j < 17):
            arquivo.write("{},".format(dados.iloc[i,j]))
        else:
            arquivo.write("{}".format(dados.iloc[i,j]))
    arquivo.write("\n")
arquivo.close() """
